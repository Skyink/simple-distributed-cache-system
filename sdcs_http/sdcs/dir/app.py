...
def update_cache():  # put application's code here
    ...
    for j in range(0, server_cnt):
        if len(dict_kv_list_to_update[j]) == 0:
            break
        # 构建请求地址
        request_url = server_url[j] + "/save_to_cache"
        print("request to " + request_url)
        # 构建请求参数
        request_json = json.dumps(dict_kv_list_to_update[j])
        # 解析响应
        headers = {"Content-Type": "application/json", "Accept-Charset": "UTF-8"}
        response = requests.post(request_url, request_json, headers=headers)
        if response.status_code != 200:
            update_result = "server id = " + str(j) + " error!"
            print(update_result)
            return update_result, 400
        print("server id = " + str(j) + " response: \n" + response.text)
